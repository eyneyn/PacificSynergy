    ...
    <form method="GET" action="{{ route('report.production_report') }}" class="flex items-center space-x-3">
        <div class="relative">
            ...
            <input type="date" name="search" id="table-search"
                   ...
                   class="block pl-10 pr-4 py-2 text-sm text-white border border-[#242c67] rounded-lg w-64 bg-[#242c67] focus:ring-[#242c67] focus:border-[#242c67]">
        </div>

        <!-- Submit Button -->
        <button type="submit"
                class="px-4 py-2 text-sm font-medium text-white bg-[#35408e] rounded-md hover:bg-[#242c67] transition-all">
            Search
        </button>
    </form>

    <!-- New Report Button -->
    <a href="{{ url('addProduction') }}"
            ...
            <tbody>
                ...
                @forelse ($reports as $report)
                    <tr class="bg-white border-b border-[#35408e] hover:bg-[#e5f4ff]">
                        <th class="px-6 py-4 font-medium text-[#35408e] whitespace-nowrap">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('report.view_report', ['id' => $report->id]) }}" class="font-medium text-blue-600 hover:underline">View</a>
                        </td>
                    </tr>
                @empty
    <tr>
